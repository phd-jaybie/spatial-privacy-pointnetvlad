...
TRAIN_FILE = 'generating_queries/3d_4m_jittered_training_queries_4096.pickle'
TEST_FILE = 'generating_queries/3d_4m_jittered_test_queries_4096.pickle'
model_file = "model_3d_4m_jittered_4096.ckpt"

LOG_DIR = FLAGS.log_dir
if not os.path.exists(LOG_DIR): os.mkdir(LOG_DIR)
LOG_FOUT = open(os.path.join(LOG_DIR, 'log_train_3d_jittered.txt'), 'w')
...
def train_one_epoch(sess, ops, train_writer, test_writer, epoch, saver):
    ...
    for i in range(len(train_file_idxs)//BATCH_NUM_QUERIES):
        ...
        if(i%3000==101):
            save_path = saver.save(sess, os.path.join(LOG_DIR, model_file))
