...
RESULTS_FOLDER="results/jittered/"
if not os.path.exists(RESULTS_FOLDER): os.mkdir(RESULTS_FOLDER)

DATABASE_FILE= 'queries/3d_jittered_spaces_evaluation_database.pickle' #3d_spaces_evaluation_database
#QUERY_FILE= 'generating_queries/spatial_privacy_evaluation_query.pickle'
        
LOG_DIR = 'log/'
MODEL_DIR = 'baseline/'
model_file= "model_3d_4m_jittered.ckpt" #"model_baseline.ckpt"#

pickle_file = 'results_jittered_model.pickle'
...
def evaluate(QUERY_SETS, output_file):
    ...
    with tf.Graph().as_default():
        ...
        results_pickle.append([
            radius,
            np.asarray(1-0.01*ave_recall),
            np.asarray(ave_intra_dist)
        ])
...
def get_recall(sess, ops, m, n):
    ...
    for i in range(len(queries_output)):
        ...
        for j in range(len(indices[0])):
            if indices[0][j] in true_neighbors:
                ...
                intra_space_distance[j]+= distances[0][j]
                break
    ...
    mean_intra_space_distance = (np.cumsum(intra_space_distance)/float(num_evaluated))
...
if __name__ == "__main__":
    ...
    """
    partial_files=sorted(os.listdir(os.path.join('generating_queries/','partial_spaces')))
    print(len(partial_files))
    file_list = int(len(partial_files)/2)
    
    for radius in np.arange(0.25,3.1,0.25)[:file_list]:
        
        print("Doing",radius)
    """
    
    for radius in np.arange(0.25,2.1,0.25): #[.0, 2.75]:#
 
        QUERY_FILE = 'queries/3d_jittered_partial_radius_{}_evaluation_query_r15_less_overlap.pickle'.format(radius)
    
        print("Doing",QUERY_FILE)
    
        output_file = RESULTS_FOLDER +pickle_file+'_partial_spaces_'+str(radius)+'_r15.txt'

        QUERY_SETS= get_sets_dict(QUERY_FILE)

        evaluate(QUERY_SETS, output_file)
