...
TRAIN_FILE = 'generating_queries/training_queries_baseline_spatial_privacy_vertical_complete.pickle'
TEST_FILE = 'generating_queries/test_queries_baseline_spatial_privacy_vertical_complete.pickle'

LOG_DIR = FLAGS.log_dir
if not os.path.exists(LOG_DIR): os.mkdir(LOG_DIR)
LOG_FOUT = open(os.path.join(LOG_DIR, 'log_train_complete.txt'), 'w')
...
def train_one_epoch(sess, ops, train_writer, test_writer, epoch, saver):
    ...
    for i in range(len(train_file_idxs)//BATCH_NUM_QUERIES):
        ...
        if(i%3000==101):
            save_path = saver.save(sess, os.path.join(LOG_DIR, "model_vertical.ckpt"))
