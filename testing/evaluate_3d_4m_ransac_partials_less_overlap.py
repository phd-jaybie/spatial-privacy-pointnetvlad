...
RESULTS_FOLDER="results/partials_less_overlap/"
if not os.path.exists(RESULTS_FOLDER): os.mkdir(RESULTS_FOLDER)

DATABASE_FILE= 'queries/3d_spaces_evaluation_database.pickle'
...
LOG_DIR = 'log/'
MODEL_DIR = 'baseline/'
model_file= "model_3d_4m_win_small.ckpt" #"model_baseline.ckpt"#
...
def evaluate(QUERY_SETS, output_file):
    ...
    with tf.Graph().as_default():
        ...
        with open(RESULTS_FOLDER +'results_ransac.pickle','wb') as pickle_output:
...
if __name__ == "__main__":
    ...
    for radius in np.arange(0.25,1.1,0.25):
 
        QUERY_FILE = 'queries/3d_ransac_partial_radius_{}_evaluation_query_r15_less_overlap.pickle'.format(radius)
    
        print("Doing",QUERY_FILE)
    
        output_file = RESULTS_FOLDER +'results_ransac_partial_spaces_'+str(radius)+'.txt'
