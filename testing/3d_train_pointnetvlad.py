...
TRAIN_FILE = 'queries/3d_4m_training_queries_baseline_spatial_privacy.pickle'
TEST_FILE = 'queries/3d_4m_test_queries_baseline_spatial_privacy.pickle'

LOG_DIR = FLAGS.log_dir
if not os.path.exists(LOG_DIR): os.mkdir(LOG_DIR)
LOG_FOUT = open(os.path.join(LOG_DIR, 'log_train_3d.txt'), 'w')
...
def train_one_epoch(sess, ops, train_writer, test_writer, epoch, saver):
    ...
    for i in range(len(train_file_idxs)//BATCH_NUM_QUERIES):
        ...
        if(i%3000==101):
            save_path = saver.save(sess, os.path.join(LOG_DIR, "model_3d_4m.ckpt"))
